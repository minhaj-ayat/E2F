    ...
    while True:
        if giveIMSI == 1:
            ...
            with socket(AF_INET, SOCK_STREAM) as clientSocket:
                ...
                print("UE sent IMSI to Fog at time : ")
        ...
        with conn:
            ...
            if not data:
                ...
                data = data.decode()
                print(data)
                if data == '200 OK' or data == "Valid":
                    end = time.time()
                    print("User authenticated")
                    print("Time required : " + str(end-start) + ' seconds')
                    print("UE authenticated at time : ")
                    print(datetime.datetime.now())
                    giveIMSI = 1
                else:
                    with socket(AF_INET, SOCK_STREAM) as clientSocket:
                        ...
                        print("UE sent data to Auth server at time : ")
                        print(datetime.datetime.now())
                        clientSocket.close()
                        with socket(AF_INET, SOCK_STREAM) as responseSocket:
                            ...
                            with connResponse:
                                ...
                                with socket(AF_INET, SOCK_STREAM) as clientResponse:
                                    ...
                                    print("UE sent Response to Client at time : ")
                                    print(datetime.datetime.now())
